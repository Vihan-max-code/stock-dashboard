#!/usr/bin/env node
/**
 * Fetches current quotes from Alpha Vantage for the 30 tracked tickers and
 ...
 * ~25/day). This script fetches 30 tickers with a 13s gap between calls,
 * which comfortably fits inside the per-minute limit. However, 30 tickers
 * EXCEEDS the free-tier daily quota — you will need a paid Alpha Vantage
 * key (the $49.99/mo "Premium" plan allows 75 requests/minute). If you're
 * on the free tier, reduce the ticker list to 25 or fewer.
 */
...
// NOTE: at 30 tickers, one full run exceeds the free-tier daily quota
// (~25 requests/day) and takes ~6.5 minutes (30 requests x 13s spacing).
// You need a paid Alpha Vantage key for reliable daily updates at this
// ticker count. If on the free tier, some tickers at the end of the list
// may fail with a rate-limit error — they'll be retried next run.
